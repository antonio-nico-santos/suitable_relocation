...
shp_path = os.path.join(path_gpkg, "R01_21_WGS84.shp")
xlsx_path = os.path.join(path_gpkg, "R01_indicatori_2021_sezioni.xlsx")
...
gdf_completo = gdf_sezioni.merge(df_dati, on='SEZ21_ID')

# 4. Selecting the data of the communes of interest
# The PROCOM code values for Torino (1272) and Pinerolo (1191)
# Checking for string or integer
codici_comuni = ['1191', '1272', 1191, 1272]
gdf_final = gdf_completo[gdf_completo['PROCOM'].isin(codici_comuni)].copy()

# 5. Saving the data to a GeoPackage
output_path = os.path.join(path_gpkg, "censimento.gpkg")
gdf_final.to_file(output_path, layer='sezioni_censimento', driver="GPKG")
